...
```{r, load_data}
  ...
  # Only use the experience condition
...
```{r, preprocessing}
  ...
  # Akaike weights
  weights <- dcast(gof[, c("id", "model", "wAIC")], id ~ model, value.var = "wAIC")
  ...
  parameter[, model := ifelse(winner == "baseline", "base", winner)]
...
```{r, study1_parameter}
  parameter[,
    ...
    by=winner]
  tab <- parameter[, .(val = paste_meansd(val)), by = .(winner_n, par)]
...
```{r, ind_fits_cutoff}
  cutoff <- 0.4
  m_cor <- dpred[model == winner, cor(pred, value), by = id][, mean(V1)]
...
```{r echo=FALSE}
  ...
  source("fig_cogn_model_fit_qualitatively.R")
```
