...
fig <- ggplot(melt(weights, id = "id", measure = m), aes(x = factor(id, levels = weights[, .(id[order(-get(models[1]))]), keyby=winner]$V1),  y = value, fill = factor(variable, model_levels, model_labels))) +
    geom_bar(stat = "identity", color = "white", size = 0.01) +
    geom_hline(yintercept = c(.50), linetype = 2, color = "white", alpha = 0.8, size = 0.4) +
    scale_fill_manual("Model", values = model_colors, labels = parse_format()) +
