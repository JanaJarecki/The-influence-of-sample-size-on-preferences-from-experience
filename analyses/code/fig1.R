...
plot_belief_updating <- function(px, N = 10000) {
  order <- ifelse(domain == "giain", 1:2, 2:1)
  x <- c(px * 5, px * 30)
  labels <- c("",
    paste0("Small sample (", x[1], "/5 ", domain, "s)"),
    paste0("Large sample (", x[2], "/30 ", domain, "s)"))
  ...
  p <- ggplot(d, aes(x = value, y = 0, fill = type, color = type, shape = type_p)) +
    geom_vline(aes(xintercept = px, linetype = paste("True Pr(gain) =", px)),color = "grey70") +
...
N <- 50000
p_02 <- plot_belief_updating(0.2, N = N) +ggtitle("Bayesian model learning about 20% Pr(gain)")
p_80 <- plot_belief_updating(0.8, N = N) +ggtitle("Bayesian model learning about 80% Pr(gain)") +guides(fill = "none", shape = "none", color = "none")

p_02 + p_80 +plot_layout(nrow = 2) +plot_annotation(tag_levels = "a")

ggsave("../figures/fig1.png", width = 7, height = 3.8)