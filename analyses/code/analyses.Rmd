...
# Für Studie a ist "nur" dataframe aber keine Rohdaten vorhancen

c <- read.csv2("../../data/processed/study1.csv")
...
c$rating <- as.numeric(paste(c$rating))
c$ev <- c$p *c$gain
c$id = c$id -4
c$id <- as.factor(c$id)
...
for(i in unique(descriptives$gamble)){
  for(s in c("us", "s", "m", "l")){
    descriptives$"D-E"[ descriptives$gamble == i &  descriptives$s == s] =
      round(descriptives$mean[descriptives$gamble == i &  descriptives$s == s] - descriptives$mean[ descriptives$gamble == i &  descriptives$s == "d"] ,2)
