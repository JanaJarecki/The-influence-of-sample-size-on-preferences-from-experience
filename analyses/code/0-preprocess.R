...
# gambles <- data.frame(
#   gambleid = 1:6,
#   type = c(rep("$-bet", 3), rep("p-bet",3)),
#   x = c(16,12,28,4,2.40,4.70),
#   pr = c(0.20,0.17,0.14,0.80,0.83,0.86),
#   ev = c(3.20,2,4,3.20,2,4.03),
#   xs = c(5,6,7,5,6,7),
#   s = c(10,12,14,10,12,14),
#   m = c(15,18,21,15,18,21),
#   l = c(30,36,42,30,36,42)
#   )

load("../../data/raw/data_analysis_A.RData")
sd <- as.data.table(sd)
sd <- sd[, c("id", "block", "trial", "SS", "p", "gain", "rating", "gamble", "s", "ptypeprob")]
setnames(sd, c("id", "condition", "trial", "samplesize", "gamblep", "gamblex", "value", "gambleid", "samplesizecat", "gambletype"))

co <- as.data.table(conf)
co <- co[, c("id", "block", "trial", "SS", "p", "gain", "rating", "gamble", "s", "ptypeprob")]
...
setcolorder(d, c("id", "condition", "trial", "gambleid", "gambletype", "gamblep", "gamblex", "samplesizecat", "samplesize", "count_x", "value", "confidence"))
