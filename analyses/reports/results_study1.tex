...
the additional predictors sample size (\(BF_{01} = 425\)) and a
...
half of the sampling phase nor those sampled during the second half of the sampling phase predict the observed evaluations. A linear model of the
...
<<<<<<< Updated upstream
lower on the basis of experience (\(M=2.74, SD=1.09\)) compared to description
...
gap observed in choice paradigms. In Study 1, participants valued
gambles as if they overweighted rare events from description
\textit{and} experience. This effect was even stronger when participants made
...
\ \ \ E & s & 14 & 6.70 & 8.40 & -1.83 & 928\\
\ \ \ E & m & 21 & 6.20 & 7.92 & -1.35 & 134\\
\ \ \ E & l & 42 & 6.00 & 7.68 & -1.11 & 17\\
\ \ \ D & -- & -- & 5.00 & 6.57 & -- & --\\
Gamble ID 4 (p-bet) &  &  &  &  &  & \\
...
<<<<<<< Updated upstream
By modeling people's choice with the two models we proposed, it is possible to examine the effect of sample size on people's judgments. We tested how good the  
\added[id=jj]{relative frequency} (RF) model and the
\added[id=jj]{Bayesian value updating} (BVU) model can explain people's judgments. To check the psychological plausibility of both models we also checked whether both models outperform a baseline model, which predicts a constant evaluation equal to the mean individual evaluation.

\textit{Modeling Procedure.} To estimate the models, the observed
...
likelihood was used to estimate the parameters of the model at the
individual level, assuming that observations follow a truncated normal
=======
...
<<<<<<< Updated upstream
\added[id=jj]{($M r\textsubscript{pred,obs} = 0.71$), except in four cases, where even the winning model fails to resemble the data qualitatively (participants s05, s19, s24, s38, with $r\textsubscript{pred,obs} < 0.40$).} \footnote{As robustness check we repeated the model comparison with subjective probability weighting, using Prelec’s single parameter weighting function (provide reference here). This weighting function incorporates non-linearities in the perception of probabilities. However, the quantitative results of the probability weighting model and the utility model, we favored a utility model without probability weighting.}
=======
