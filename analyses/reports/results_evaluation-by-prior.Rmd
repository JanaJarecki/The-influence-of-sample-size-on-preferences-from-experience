...
\added[id=jj]{Regarding uncertainty about beliefs, the Bayesian model predicts that more evidence decreases the uncertainty about beliefs. Thus confidence should increase with sample size for Bayesian learners. The relative frequency model makes no predictions about confidence. To test this, we classified participants into Bayesian and relative-frequency learners based on the best-fitting model. Mean confidence ratings, z-standardized within participants, did not grow from the extra small sample size ($`r d[samplesizecat == "xs", paste_msd(conf_scaled, label = T)]`$), to the small sample size ($`r d[samplesizecat == "s", paste_msd(conf_scaled, label = T)]`$), medium sample size ($`r d[samplesizecat == "m", paste_msd(conf_scaled, label = T)]`$), and large sample size ($`r d[samplesizecat == "l", paste_msd(conf_scaled, label = T)]`$, Figure \ref{fig:fig6}b), a linear regression model ($M_0$) without the cognitive model classification as predictor but with predictors sample size category and gambletype outperformed a regression including cognitive model category as predictor ($BF_{01}`r BF_conf_prior`$).}