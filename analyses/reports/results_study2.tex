...
(\(M_0\)) outperformed models with the added predictor sample size
(\(BF_{01} = 385\)) and with a sample size x gamble type interaction
(\(BF_{02} > 1000\); models with by-participant random effects). Unlike
in Study 1 there was no substantial difference between the evaluation of
\$-bet gambles (\(M=4.62, SD=3.01\)) and p-bet gambles
(\(M=5.17, SD=4.87\)); the data equally supported a model including
gamble type as predictor (\(M_0\)) and one without gamble type as
predictor, \(BF_{01} = 1.48\) (both models include by-participant and
by-expected-value random effects and use the valuations as the dependent
variable).
...
described slightly more than half of the participants best (8 of 40;
20\%). The relative frequency model described 31 participants best
(78\%). The evidence strength of the models is shown in Figure
\ref{fig:fig4}. The models' mean Bayesian information criterion across
all participants equaled BIC\textsubscript{BVU}\(= -79\),
BIC\textsubscript{RF}\(= -83\), and BIC\textsubscript{BASE}\(= 25\)
(lower values indicate better fit).
\added[id = jj2]{In Study 2, the Bayesian updating model needed the learning rate parameter $\delta$ to describe participants well.}\footnote{\added[id=jj2]{A constrained version of the Bayesian value updating model with learning rate $\delta=1$ described only 7 of 40 (18\%) participants best; the relative frequency model described 32 participants best (80\%); the baseline model described 1 participants best.}}

\begin{figure}[htb]


<<model_comparison, fig.height=5, echo=F>>=

R$fig_modelcomparison
...
\midrule
BASE (\textit{n}$=$1) & -- & -- & -- & 1.00 (NA)\\
BVU (\textit{n}$=$31) & 2.19 (1.46) & 0.15 (0.19) & 0.52 (0.58) & 0.16 (0.04)\\
\bottomrule
\addlinespace
\end{tabular}
...
(\(M_{\tau}= 2.19\)) and those best described by the relative frequency
model (\(M_{\alpha}=3.03\)), \(\Delta\) \(M = -0.56\) 95\% HDI
\([-3.56\), \(2.28]\), \(\mathrm{BF}_{\textrm{01}} = 2.50\). The prior
belief about the probability of a positive outcome, for participants
that were best described the BVU model, was \(B = 0.26\) (resulting from
\(\alpha = 0.52\) and \(\beta_0 = 1.48\)) and the associated learning
rate \(\delta\) was \(M_{\delta}=0.15\), indicating faster belief
...
\(r\textsubscript{pred,obs} = 0.71\)), except in four cases
