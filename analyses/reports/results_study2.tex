...
(\(BF_{01}= 363\)) and a sample-size-gamble-type interaction
(\(BF_{02} > 1000\); models with by-participant random effects). As in
Study 1, higher value were assigned to \$-bet gambles
(\(M=4.69, SD=3.10\)) compared to the values assigned to p-bet gambles
(\(M=5.25, SD=4.94\)); statistical analyses favored a model including
gamble type as predictor (\(M_0\)) over one without gamble type as
predictor, \(BF_{01} =1.48\) (both models include by-participant and
by-expected-value random effects and use the valuations as criterion).
These results corroborate the aggregate findings of Study 1.
...
sample sizes (Table \ref{tab:means_study1}, rightmost column). The
\$-bets were evaluated higher from experience (\(M=4.69, SD=3.10\))
compared to description (\(NA\)), and the p-bets were evaluated lower
from experience compared (\(M=5.25, SD=4.94\)) to description (\(NA\)).
This D--E gap replicates the results from Study 1: participants valued
gambles as if they overweight rare events from description \textit{and}
experience.
