...
\noindent\added[id=jj]{where $n_t$ represents the number of gain outcomes and $n_0$ represents the number of zero outcomes observed up to outcome number $t$. The parameter $\delta$ is a learning rate that scales the number of observations and can account for individual differences in the revision of prior beliefs \citep{Edwards1967,Tauber2017,Jarecki2017}.  As point estimate of people's (posterior) beliefs about the probability of the gain outcome, we use the expected value of the posterior distribution, that follows  this distribution${B}_\mathrm{BVU, t} = \frac{\alpha_t}{\alpha_t + \beta_t}$.}

\added[id=jj]{The learning rate adjusts the effect that new information has on the prior beliefs. Conservative learners ($\delta < 1$) under-weight the incoming observations and revise their prior beliefs insufficiently relative to Bayes' theorem. Liberal learners ($\delta > 1$) overwrite prior beliefs quicker with the experienced observations compared to optimal Bayesian updating of prior beliefs ($\delta = 1$). Thus the learning rate governs the degree to which learners revise their prior beliefs about the probabilities of risky outcomes after the observations of outcomes.}
