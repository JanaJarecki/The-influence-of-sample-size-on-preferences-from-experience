...
\abstract{People often evaluate risky options in situations in which they need to learn from experience how likely the options' outcomes are. When people can actively sample information about outcomes and outcome probabilities to make decisions, they typically draw small samples. \added[id=jj]{We examine how the number of samples affects preferences for risky gambles in evaluations from experience. In two studies, people drew samples of different sizes from risky gambles and indicated their selling prices for these gambles and their confidence in their selling-price estimation}. Remarkably, the results show that, on average, neither valuations nor confidence in the valuations changed substantially with sample size. However, \added[id=jj]{cognitive modeling of individual-level learning showed that}  \added[id=jj]{most} participants could be classified as Bayesian learners with prior beliefs about outcomes, and the minority adhered to a frequentist learning strategy. Valuations of Bayesian learners changed with sample size as predicted by Bayesian principles. In contrast, sample size did not influence valuations of frequentist learners. These results illustrate the variability in how people learn from sampled information and provide an explanation for why sample size often does not affect judgments. 
}
...
\added[id=jj]{The \textit{relative frequency model (RF)} estimates probabilities from relative frequencies. It formalizes how people, who trust in the law of small numbers, estimate probabilities. Accordingly people estimate the probability of gains from the observed relative frequencies of gains.} This model predicts that after sampling $t$ times the belief about the gain probability corresponds to
$${B}_{\mathrm{RF}, t}  =  \frac{1}{t} \times n_t,$$
\added[id=jj]{where $n_t$ represents the number of gains observed until $t$.}

\added[id=jj]{The relative frequency model suggests that changes in sample size do not influence valuations unless the sample size changes the observed relative frequencies. The relative frequency model does not incorporate prior beliefs about the probabilities of the outcomes.}

\subsubsection{Bayesian value updating model}
The \emph{Bayesian value updating (BVU)} model implements a Bayesian perspective on probability learning and the resulting uncertainty from sampling. \added[id=jj]{According to Bayesian principles both the observed frequencies of gains and prior beliefs about outcome probabilities influence participants' final belief about the probabilities. The Bayesian model formalizes the beliefs about gain probabilities by a beta distribution ${B}_{\mathrm{BVU}} \sim Beta(a,b)$. Initially (at $t = 0$) beliefs are represented by a prior belief distribution $B_{\mathrm{BVU},0} \sim Beta(\theta_G$, $\theta_0)$, with person-specific parameters $\theta$. The prior belief is updated from the sampled number of gains and zero-outcomes; the updated beliefs after the $t$th sample follow the distribution}

$$B_{\mathrm{BVU},t} \sim Beta(\theta_G + \delta n_t, \theta_0 + \delta m_t)$$,

\added[id=jj]{where  $\delta$ is a learning rate to account for individual differences in conservatism \citep{Edwards1967,Tauber2017}; the values $n_t, m_t$ are the counts of gain outcomes and zero-outcomes, respectively, after sample $t$, and $\theta_G$, $\theta_0$ are the gain and zero-outcome priors, respectively. The point estimate for the updated belief about the gain probability equals the posterior mean of this distribution, ${B}_\mathrm{BVU, t} = \frac{A}{A + B}$, with $A$ being a short-hand for $\theta_G + \delta b_t$ (and respectively $B=\theta_0 + \delta m_t$).}

\textcolor{blue}{The updating process is illustrated in Figure \ref{fig:sensi} (see also below).}

\subsubsection{Utility Function}
\added[id=jj]{Learners represent the value of a gambles by its subjective expected value $V$, which is the product of belief about the gamble's probabilities after sampling and the utility of the outcome of the gamble. We used a power utility, $u(g) = g^{\mathrm{\alpha}}$, where $g$ is the gain outcome and $\alpha$ is a free parameter. Thus, the predicted evaluation by the relative frequency (RF) and Bayesian updating (BVU) model after sample $t$ is given by }
...
% \added{The cognitive modeling results help to understand the null finding---that sample size has no effect on evaluations---that we observed at the aggregate level. First, the aggregate analysis fails to take the heterogeneity in strategy use into account. Secondly, the aggregate analysis fails to account for prior beliefs about gain probabilities.}
...
The mean and median ratings in Table \ref{table:meansStudy2} show that valuations for \$-bets (Gamble IDs 1--3) were higher than for p-bets (Gamble IDs 4--6), despite the gambles having the same expected value. This finding is supported by comparing a linear Bayesian regression model of gamble evaluations $\mathrm{M}\textsubscript{0}$ \added[id=jj]{that includes the expected value as random effect} with a model $\mathrm{M}\textsubscript{1}$ \added[id=jj]{that includes p-bets and \$-bets as fixed effects} ($BF\textsubscript{10} > 1,000$).
