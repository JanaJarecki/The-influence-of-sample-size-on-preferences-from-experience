...
\added[id=jj]{Learners represent the value of a gambles by its subjective expected value $V$, which is the product of belief about the gamble's probabilities after sampling and the utility of the outcome of the gamble. We used a power utility, $u(g) = g^{\mathrm{\alpha}}$, where $g$ is the gain outcome and $\alpha$ is a free parameter. The predicted valuation by the model $m$ ($\in$ RF, BVU) after sample $t$ is given by }
