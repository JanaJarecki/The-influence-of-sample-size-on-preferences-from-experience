...
We tested whether individuals integrate samples by following Bayesian principles or by following the belief in the law of small numbers in the context of experience-based risky choice. For this purpose, we formalized two models that mathematically implement the two psychological principles described above. 
...
\added[id=jj]{The \textit{relative frequency model (RF)} forms beliefs about probabilities from relative frequencies. It formalizes how people, who trust in the law of small numbers, estimate probabilities. Accordingly people rely only on the observed relative frequencies of gains.} This model predicts that after sampling $t$ times the belief about the probability of a gain corresponds to
$${B}_{\mathrm{RF}, t}  =  \frac{1}{t} \times n_t,$$
\added[id=jj]{where $n_t$ represents the number of gains observed until sample numner $t$ (including $t$).}
...
$$B_{\mathrm{BVU},t} \sim Beta(\theta_G + \delta n_t, \theta_0 + \delta m_t)$$,

\added[id=jj]{where  $\delta$ is a learning rate to account for individual differences in conservatism \citep{Edwards1967,Tauber2017}; the values $n_t, m_t$ are the counts of gain outcomes and zero-outcomes, respectively, after sample $t$; parameter $\theta_G$, $\theta_0$ are the shape parameter of the prior Beta distribution for gains and zero-outcomes, respectively. The point estimate for the updated belief about the gain probability equals the posterior mean of this distribution, ${B}_\mathrm{BVU, t} = \frac{A}{A + B}$, with $A$ being a short-hand for $\theta_G + \delta b_t$ (and respectively $B=\theta_0 + \delta m_t$).}

The Bayesian (BVU) model's updating process is illustrated in Figure \ref{fig:sensi}\added[id=jj]{, showing the model's beliefs about the probability of drawing a gain as a function of the number of samples that were drawn and the initial beliefs about the gain probability. The orange curves show different initial beliefs, the purple and black curve illustrate the change in belief after observing a small sample of outcomes (\textit{n}$=$5) and a large sample (\textit{n}$=$30). The beliefs after observing the large sample approximate the true probability of the gamble, independently of the prior belief. For small samples, however, the prior belief impacts the models' posterior belief about the gamble's gain probability (purple curve). Unlike the Bayesian model, the relative frequency model predicts a constant probability $Pr(\text{gain})$ equal to the true $Pr(\text{gain})$after having seen a representative sequence of outcomes, such as 1 in 5 gains or 6 in 30 gains.}
...
\added[id=jj]{Both models assume that after sampling from a gamble, it's value is a subjective expected value $V$, which is the product of the belief about the gamble's probabilities after sampling and the utility of the outcome of the gamble. We used a power utility, $u(g) = g^{\mathrm{\alpha}}$, where $g$ is the gain outcome and $\alpha$ is a free parameter. The predicted valuation by the model $m$ ($\in$ RF, BVU) after sample $t$ is given by }

$$V_{\mathrm{m}, t} = \sqrt[\mathrm{\alpha}]{{B}_{\mathrm{m}, t} \times u({g})}$$
...
\input{analyses/code/study1_results}
...
\input{analyses/code/study2_results}
